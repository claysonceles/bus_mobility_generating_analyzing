...
CURRENT_TIMEZONE = 'America/Winnipeg'
CURRENT_DAY = '2020-08-10'
...
def get_trajectories(ShapeID):
    conn = pymysql.connect(host='localhost', port=3306, user='', passwd='', db='busmobility')
    cursor = conn.cursor()
    query = "SELECT * FROM OttawaWeekday WHERE ShapeID='" + str(ShapeID)+ "'"
...
def get_all_shapes_id():
    conn = pymysql.connect(host='localhost', port=3306, user='', passwd='', db='busmobility')
    cursor = conn.cursor()
    query = "SELECT DISTINCT ShapeID FROM OttawaWeekday"
    df = pd.read_sql(query, conn)
    print(df.head())
    all_shapes_id = []
    if not df.empty:    	
    	id_list = df.values.tolist()
    	all_shapes_id = [val for sublist in id_list for val in sublist]
    cursor.close()
    conn.close()
    return all_shapes_id

def main_calibrated_trajs(shapeIDs):
	pool = mp.Pool(processes=16)
	pool.map(calibrate, shapeIDs)
	pool.close()
	pool.join()


df_stops_times = pd.read_csv("/home/clayson/globecom_extension/datasets/gtfs/winnipeg_weekday/stop_times.txt")
df_stops = pd.read_csv("/home/clayson/globecom_extension/datasets/gtfs/winnipeg_weekday/stops.txt")
df_trips = pd.read_csv("/home/clayson/globecom_extension/datasets/gtfs/winnipeg_weekday/trips.txt")
df_shapes = pd.read_csv("/home/clayson/globecom_extension/datasets/gtfs/winnipeg_weekday/shapes.txt")
...
def get_bus_stops_dataframe(trip_id):
	...
	return df_bus_stops[['trip_id', 'arrival_time', 'departure_time', 'stop_id', 'stop_sequence', 'stop_code', 'stop_name', 'stop_lat', 'stop_lon']]
...
def calibrate(trip_id):
	try:
		...
		current_shape_id = current_trip_list[0][6] #get the shape_id value for this trip_id
		...
		for j in range(1, len(route_shape_stops_aslist)):
			...
			goal_point = (current_sample[0], current_sample[1], current_sample[7], current_sample[8])
			anchor_index = -1      
			...
			previous_point_utm = utm.from_latlon(previous_sample[7], previous_sample[8])        
			d = euclidean_dist(previous_point_utm, goal_point_utm)
			points_list = [(previous_sample[7], previous_sample[8])]
			for anchor_point in anchors: 
				points_list.append((anchor_point[2], anchor_point[3]))
			points_list.append((current_sample[7], current_sample[8]))
			...
			if len(anchors) > 0:
				while elapsed_distance < total_distance: 
					...
					if anchor_index == len(anchors)-1:
						goal_point_utm = utm.from_latlon(current_sample[7], current_sample[8])
				...
				while temporal_count > 0:
					...
					j = j + 1
					temporal_count = temporal_count - 1
		final_point_utm = utm.from_latlon(current_sample[7], current_sample[8])
		...
		calibrated_traj_latlong[['trip_id','timestamp', 'latitude', 'longitude', 'speed', 'stop_id','stop_name','route_id', 'trip_headsign', 'direction_id', 'shape_id', 'calibrated']].to_csv("/home/clayson/globecom_extension/datasets/calibrated/winnipeg_weekday/calibrated_traj_"+str(trip_id)+"_.csv", index=False)
		#calibrated_traj_latlong[['trip_id','timestamp', 'latitude', 'longitude', 'speed', 'stop_id','stop_name','route_id', 'trip_headsign', 'direction_id', 'shape_id', 'calibrated']].to_csv("/home/clayson/globecom_extension/datasets/calibrated/ottawa_weekday/calibrated_traj_"+trip_id+"_.csv", index=False)
