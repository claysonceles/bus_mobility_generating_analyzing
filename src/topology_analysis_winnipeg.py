...
def get_instantaneous_graph(df, timestamp, radius, path_filename):
	...
	for row in raw_data_matrix:
		G.add_node(row[0], lat=row[2], lon=row[3], route_id=row[7], trip_headsign=row[8], direction_id=row[9], shape_id=row[10])
...
def create_graph(timestamp):
	radius = 500
	path_filename = "/home/clayson/globecom_extension/graphs/winnipeg_weekdays_500m/"
	string_query = "SELECT * FROM WinnipegBusWeekday20200810 WHERE timestamp="+str(timestamp)+";"
...
def compute_topology_metrics(graph_timestamp):
    values = []
    from_directory = "/home/clayson/globecom_extension/graphs/winnipeg_weekdays_500m/"+str(graph_timestamp)+".gpickle"
    ...
    to_directory = "/home/clayson/globecom_extension/data_results/winnipeg_weekday/topology_metrics_500m/"
...
def get_mid_point(positions):
    ...
    lat_mid, lon_mid = utm.to_latlon(avgx, avgy, 18, 'T')
    return (lat_mid, lon_mid)


def get_components_positions():
	from_directory = "/home/clayson/globecom_extension/graphs/ottawa_weekdays_300m/1600117200.gpickle"
	G = nx.read_gpickle(from_directory)
	...
	position_components.to_csv("/home/clayson/globecom_extension/data_results/ottawa_weekday/components_geo_positions/300m/compute_geo_comp_ottawa_5_pm_300m.csv", sep=',', index=False)
...
def compute_size_of_components(graph_timestamp):
    ...
    to_directory = "/home/clayson/globecom_extension/data_results/winnipeg_weekday/size_of_components/500m/"
    write_list2csv_by_line(to_directory+str(graph_timestamp)+".csv", size_of_components)


def get_contact_nodes(interval):
	#interval = (1600078200, 1600078299) 
	directory = "/home/clayson/globecom_extension/graphs/ottawa_weekdays_500m/"
	edges = {}
	...
	with open('/home/clayson/globecom_extension/data_results/ottawa_weekday/contacts/nodes/contact_time_ottawa_weekday_500m.csv', 'w') as csv_file:  
...
def compute_contact_characteristics(origin_directory, destination_directory):
	...
	write_list2csv_by_line(destination_directory+"contact_time_ottawa_weekday_500m.csv", contact_duration_list)
...
def get_degree_all_nodes(graph_timestamp):
	...
	to_directory = "/home/clayson/globecom_extension/data_results/winnipeg_weekday/degrees/500m/"
	write_list2csv_by_line(to_directory+str(graph_timestamp)+".csv", degrees)


def main_create_graphs(interval_timestamp_winnipeg_weekday):
	pool = mp.Pool(processes=20)
	pool.map(create_graph, interval_timestamp_winnipeg_weekday)
	pool.close()
	pool.join()


def main_compute_topology_metrics(interval_timestamp_winnipeg_weekday):
	pool = mp.Pool(processes=20)
	pool.map(compute_topology_metrics, interval_timestamp_winnipeg_weekday)
	pool.close()
	pool.join()


def main_compute_size_of_components(interval_timestamp_winnipeg_weekday):
	pool = mp.Pool(processes=20)
	pool.map(compute_size_of_components, interval_timestamp_winnipeg_weekday)
	pool.close()
	pool.join()


def main_compute_all_degrees(interval_timestamp_winnipeg_weekday):
	pool = mp.Pool(processes=20)
	pool.map(get_degree_all_nodes, interval_timestamp_winnipeg_weekday)
	pool.close()
	pool.join()


if __name__ == "__main__":
	min_max_timestamp = (1597053000, 1597121940)
	interval_timestamp_winnipeg_weekday = range(1597053000, 1597121940)
	#main_create_graphs(interval_timestamp_winnipeg_weekday)
	#main_compute_topology_metrics(interval_timestamp_winnipeg_weekday)
	#main_compute_size_of_components(interval_timestamp_winnipeg_weekday)
	main_compute_all_degrees(interval_timestamp_winnipeg_weekday)
	#get_components_positions()
	#get_contact_nodes(min_max_timestamp)
	#get_contact_bus_lines(min_max_timestamp)
	#origin_directory = "/home/clayson/globecom_extension/data_results/ottawa_weekday/contacts/nodes/contact_time_ottawa_weekday_500m.csv"
	#destination_directory = "/home/clayson/globecom_extension/data_results/ottawa_weekday/contacts/nodes/metrics/"
